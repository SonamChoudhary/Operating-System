...
/* ================================================================ */
static wait_queue_head_t dev_wait_q[SLEEPY_NDEVICES];   //queue and conditional flags
static int flag [SLEEPY_NDEVICES] = {0};
	    ...
	    loff_t *f_pos)
{
  ...
  /* YOUR CODE HERE */
   dev_no = MINOR(dev->cdev.dev);
  flag[dev_no]=1;
  wake_up_all (&dev_wait_q[dev_no]);
	     ...
	     loff_t *f_pos)
{
  ...
  {       
        flag[dev_no]=0;
	mutex_unlock (&dev->sleepy_mutex);
	retval = wait_event_timeout(dev_wait_q[dev_no],flag[dev_no]!=0,sleep_time*HZ)/HZ;
	if (mutex_lock_killable(&dev->sleepy_mutex))
    		return -EINTR;
  }		
  /* END YOUR CODE */
...
sleepy_init_module(void)
{
  ...
  for (i = 0; i < sleepy_ndevices; ++i) {
	  init_waitqueue_head(&dev_wait_q[i]); //initialising device wait queue
